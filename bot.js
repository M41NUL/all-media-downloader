...
bot.on('text', async (ctx) => {
  ...
  // ── Step 2: Download progress bar ────────────────────────────────────────
  const dlSteps = config.PROGRESS.DOWNLOAD_STEPS.map(([p, s]) => [p, s]);
  await safeEdit(ctx, chatId, statusMsg.message_id,
    progressBar(0, '0 MB/s', 'download'),
    { parse_mode: 'MarkdownV2' }
  );

  // Start real download in parallel with animation
  const downloadPromise = download(text, sess.platform || null);
  await animateProgress(ctx, chatId, statusMsg.message_id, dlSteps, 'download');

  let info;
  try {
    info = await downloadPromise;
  } catch (err) {
